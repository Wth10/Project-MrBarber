...
function obterServicos() {
    $pdo = Banco::conectar();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $consulta_sql_Servico = "SELECT DESCRICAO FROM servico WHERE STATUS LIKE '0' ";
    Banco::desconectar();
    $resultadoServico = 0;
    foreach($pdo -> query($consulta_sql_Servico) as $row) {
        $resultadoServico = $resultadoServico.'<option>'. $row['DESCRICAO'] .'</option>';
    }
